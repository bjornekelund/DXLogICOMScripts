...
// switching, CI-V based serial switching may not be sufficiently fast. 
// By Björn Ekelund SM7IUN bjorn@ekelund.nu 2018-12-19
    ...
    public class IcomBandPower : ScriptClass
    {
        ...
        private void HandleBandChange(int RadioNumber)
        {
            ...
            if ((RadioNumber == 1) || (RadioNumber == 2)) // If a regular band change 
            {
                ...
                usedPower = (int)((255.0 * TxPower[usedRadioIndex, megaHertz]) / 100.0 + 0.5);
        ...
        private void HandleFocusChange()
        {
            ...
            if (cdata.OPTechnique == 4) // No need to check for ICOM radio, handler is not active if not
            {  // If SO2V, only physical radio 1 is used
                ...
                usedPower = (int)((255.0 * TxPower[0, megaHertz]) / 100.0 + 0.5);
                if (usedPower > 255) usedPower = 255;
                ...
                if (Debug)
                    frmMain.SetMainStatusText(
                        string.Format("IcomBandPower: Focus change: Controlled radio #{0} Focused Radio #{1} megaHertz {2} Power {3}% UsedPower {4} Command: [{5}]",
                        1, cdata.FocusedRadio, megaHertz, TxPower[0, megaHertz],
                        usedPower, BitConverter.ToString(IcomSetPower)));
